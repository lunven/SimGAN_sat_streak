...
from simgan import *
 #python generate_refined_images.py --o "multi" --i "refined_test.npy" --k "target_test.npy"
...
def main(args, seed = 123456789):
    ...
    refiner_model = keras.models.load_model('model/refiner.h5',custom_objects={'self_regularization_loss': self_regularization_loss})
    ...
    a = np.array(satellite)
    c = np.floor(np.shape(s_img)[0]/np.shape(s_imgs)[0])
